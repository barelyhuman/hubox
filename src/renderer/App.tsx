...
import type { HuBoxNotification, NotificationDetails, NotificationStats } from './github'
...
const DEFAULT_STATS: NotificationStats = {
  total: 0, unread: 0, appUnread: 0, done: 0, inProgress: 0, lastSync: 0, isOnline: false,
}
...
export const App = () => {
    ...
    try {
      const [notifs, newStats] = await Promise.all([
        v === 'inbox'
          ...
          : window.githubAPI.getAll().then(all =>
              v === 'done' ? all.filter(n => n.isDone) : all
            ),
  ...
  useEffect(() => {
    window.tokenAPI.get().then(async token => {
      if (token) {
        try {
  ...
  const handleTokenSaved = useCallback(async (token: string) => {
    try {
      await initializeApp(token)
    } catch (error) {
  ...
  if (screen === 'token-setup') return <TokenSetup onTokenSaved={handleTokenSaved} />

  const selected = notifications.find(n => n.id === selectedId) || null

  return (
    <div class="app-layout">
      <Sidebar activeView={activeView} onNavigate={setActiveView} stats={stats} onLogout={handleLogout} />
      <NotificationList title={VIEW_TITLES[activeView]} notifications={notifications} selectedId={selectedId} onSelect={setSelectedId} />
      <DetailPane notification={selected} details={details} onMarkDone={handleMarkDone} />
    </div>
  )
}
